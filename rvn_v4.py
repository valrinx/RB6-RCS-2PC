"""
RVN — Recoil Control System  v4.0
Changes from v3.1:
  • NEW: Vertical Delay + Vertical Duration (mirrors horizontal timing system)
  • NEW: Game-agnostic config system — free-form tags (gun, attachment, game, etc.)
        No more R6-only restriction. Works with Rust, CS2, Valorant, any game.
  • IMPROVED: Recoil Curve UX — green reference line, fill, clearer canvas
  • IMPROVED: Browse panel — tag-chip filters auto-built from saved configs
  • KEEP: All v3 controller logic (MAKCU / KMBox / Software), humanize, curves
"""
...
class SoftwareController:
    ...
    def __init__(self):
        self._user32 = None; self._ready = False
        self._btn: dict = {}; self._lock = threading.Lock()
        self._connected = False
        if _HAS_CTYPES:
            try: self._user32 = ctypes.windll.user32; self._ready = True
        ...
        if self._ready:
            ...
            self._MOUSEINPUT=MOUSEINPUT; self._INPUT=INPUT; self._INPUT_sz=ctypes.sizeof(INPUT)

    def is_connected(self): return self._connected
    def connect(self):
        self._connected=True
        print(f"[Software] Ready ({'ctypes SendInput' if self._ready else 'no-op (non-Windows)'})")
        return True
    ...
    def StartButtonListener(self):
        ...
        def _poll():
            while self._connected:
                with self._lock:
                    for n,vk in self._VK.items(): self._btn[n]=bool(self._user32.GetAsyncKeyState(vk)&0x8000)
                time.sleep(0.005)
    ...
    def simple_move_mouse(self,x,y):
        ...
        except Exception as e: print(f"[Software] move error: {e}")
...
class KMBoxController:
    CMD_MOVE=0x0001; CMD_MONITOR=0x0100; CMD_CONNECT=0x000E
    _BTN={"LMB":1,"RMB":2,"MMB":4,"M4":8,"M5":16}

    def __init__(self):
        self._sock=None; self._connected=False
        self._mac=b'\x00\x00\x00\x00'; self._rand=0; self._seq=0
    ...
    def connect(self):
        ...
        try:
            ...
            self._rand=random.randint(1,0xFFFFFFFF); self._seq=0
            sock=socket.socket(socket.AF_INET,socket.SOCK_DGRAM)
            sock.settimeout(2.0); self._sock=sock
            sock.sendto(self._build(self.CMD_CONNECT,b'\x00'*4),(ip,port))
            resp,_=sock.recvfrom(1024)
            if len(resp)>=16:
                self._connected=True; sock.settimeout(0.05)
                print(f"[KMBox] Connected {ip}:{port}"); return True
            raise Exception(f"Bad handshake ({len(resp)} bytes)")
        except Exception as e:
            print(f"[KMBox] Failed: {e}"); self._connected=False; return False
    ...
    def simple_move_mouse(self,x,y):
        ...
        with self._lock:
            try: self._sock.sendto(self._build(self.CMD_MOVE,struct.pack('<hhhB',x,y,0,0)),self._addr())
            except: self._connected=False

    def _build(self,cmd,payload):
        self._seq+=1
        return self._mac+struct.pack('<III',self._rand,self._seq,cmd)+payload+b'\x00'*max(0,48-len(payload))
    def _addr(self):
        c=app_state.get_kmbox_config(); return(c["ip"],int(c["port"]) if c["port"] else 1408)
...
def read_configs(config_file=None):
    f=config_file or DEFAULT_CONFIG_FILE; p=get_config_path(f)
...
def write_configs(configs,config_file=None):
    f=config_file or DEFAULT_CONFIG_FILE; p=get_config_path(f); tmp=p+".tmp"
    try:
...
class GunConfig(BaseModel):
    # v4: game-agnostic — free-form name + optional tag dict
    name:                    str  = Field(...,min_length=1,max_length=120)
    tags: Optional[Dict[str,str]] = None   # e.g. {"game":"Rust","attach":"Comp","barrel":"4x"}
    # Vertical
...
class AppState:
    ...
    def set_active_value(self,v):          self._s('active_pull_down_value',v)
    def get_active_value(self):            return self._g('active_pull_down_value')
    def set_horizontal_value(self,v):      self._s('active_horizontal_value',v)
    def get_horizontal_value(self):        return self._g('active_horizontal_value')
    def set_horizontal_delay(self,ms):     self._s('horizontal_delay_ms',max(0,min(5000,ms)))
    def get_horizontal_delay(self):        return self._g('horizontal_delay_ms')
    def set_horizontal_duration(self,ms):  self._s('horizontal_duration_ms',max(0,min(10000,ms)))
    def get_horizontal_duration(self):     return self._g('horizontal_duration_ms')
    def set_vertical_delay(self,ms):       self._s('vertical_delay_ms',max(0,min(5000,ms)))
    def get_vertical_delay(self):          return self._g('vertical_delay_ms')
    def set_vertical_duration(self,ms):    self._s('vertical_duration_ms',max(0,min(10000,ms)))
    def get_vertical_duration(self):       return self._g('vertical_duration_ms')
    def set_curves(self,pd,hz):
        with self.lock: self.pull_down_curve=pd; self.horizontal_curve=hz
    def get_curves(self):
    ...
    def set_smooth(self,v):   self._s('smooth_factor',max(0.0,min(0.95,v)))
    def get_smooth(self):     return self._g('smooth_factor')
    def get_enabled(self):    return self._g('is_enabled')
    def toggle_enabled(self):
        with self.lock: self.is_enabled=not self.is_enabled; return self.is_enabled
    ...
    def set_kmbox_config(self,ip,port,uuid):
        with self.lock: self.kmbox_ip=ip; self.kmbox_port=port; self.kmbox_uuid=uuid
    def get_kmbox_config(self):
        with self.lock: return{"ip":self.kmbox_ip,"port":self.kmbox_port,"uuid":self.kmbox_uuid}
    def get_status(self):
        with self.lock:
            return{
                "is_enabled":self.is_enabled,"toggle_button":self.toggle_button,
                "pull_down":self.active_pull_down_value,"horizontal":self.active_horizontal_value,
                "horizontal_delay_ms":self.horizontal_delay_ms,"horizontal_duration_ms":self.horizontal_duration_ms,
                "vertical_delay_ms":self.vertical_delay_ms,"vertical_duration_ms":self.vertical_duration_ms,
                "jitter_strength":self.jitter_strength,"smooth_factor":self.smooth_factor,
                "current_config_file":self.current_config_file,"trigger_mode":self.trigger_mode,
                "controller_type":self.controller_type,"kmbox_ip":self.kmbox_ip,"kmbox_port":self.kmbox_port,
...
class _Smoother:
    ...
    def update(self,tx,ty,alpha):
        self.x=alpha*tx+(1-alpha)*self.x; self.y=alpha*ty+(1-alpha)*self.y
        return int(round(self.x)),int(round(self.y))
    def reset(self): self.x=0.0; self.y=0.0

def humanize(rx,ry,jitter,smoother,smooth):
    if jitter>0:
        sc=max(abs(rx),abs(ry))*jitter
        rx+=random.gauss(0,sc*0.5); ry+=random.gauss(0,sc*0.5)
    return smoother.update(rx,ry,max(0.05,min(1.0,1.0-smooth)))


# ══════════════════════════════════════════════════════════════════════════════
#  Main control loop — vertical delay/duration support
# ══════════════════════════════════════════════════════════════════════════════
TICK_S=0.010
...
def mouse_control_loop():
    if _HAS_MAKCU: makcu_controller.StartButtonListener()
    toggle_was=False; hold_start=None; curve_tick=0; smoother=_Smoother()
    while True:
        t0=time.perf_counter()
        try:
            ctrl=get_active_controller()
            if not ctrl.is_connected():
                time.sleep(0.5); ctrl.connect(); continue
            btn=app_state.get_toggle_button(); pressed=ctrl.get_button_state(btn)
            if pressed and not toggle_was: app_state.toggle_enabled(); smoother.reset()
            toggle_was=pressed
            lmb=ctrl.get_button_state("LMB"); rmb=ctrl.get_button_state("RMB")
            fire=(lmb and rmb) if app_state.get_trigger_mode()=="LMB+RMB" else lmb
            if app_state.get_enabled() and fire:
                now=time.perf_counter()
                if hold_start is None: hold_start=now; curve_tick=0
                hold_ms=(now-hold_start)*1000.0
                pd_curve,hz_curve=app_state.get_curves()

                # ── Vertical with delay/duration ──
                ...
                if v_active:
                    raw_y=(pd_curve[min(curve_tick,len(pd_curve)-1)]/5.0 if pd_curve
                           else app_state.get_active_value()/5.0)
                else:
                    raw_y=0.0

                # ── Horizontal with delay/duration ──
                ...
                if hold_ms>=h_delay and (h_dur==0 or hold_ms<=h_delay+h_dur):
                    raw_x=(hz_curve[min(curve_tick,len(hz_curve)-1)]/5.0 if hz_curve
                           else app_state.get_horizontal_value()/5.0)

                curve_tick+=1
                mx,my=humanize(raw_x,raw_y,app_state.get_jitter(),smoother,app_state.get_smooth())
                if mx or my: ctrl.simple_move_mouse(mx,my)
            else:
                hold_start=None; curve_tick=0; smoother.reset()
        except Exception as e:
            print(f"[LOOP] {e}"); time.sleep(1); continue
        elapsed=time.perf_counter()-t0
        if (coarse:=TICK_S-elapsed-0.001)>0: time.sleep(coarse)
        ...
        while True:
            ...
            try:
                ...
                if "pull_down_curve" in msg:
                    ...
                    app_state.set_curves(v if isinstance(v,list) else None,hz)
            except json.JSONDecodeError: pass
    except WebSocketDisconnect: pass

@app.get("/status")
async def status(): return app_state.get_status()

@app.post("/toggle")
async def toggle(): return{"is_enabled":app_state.toggle_enabled()}
...
@app.get("/configs")
async def get_configs(): return read_configs(app_state.get_current_config_file())
...
async def delete_config(key:str):
    cf=app_state.get_current_config_file(); cfgs=read_configs(cf)
    if key not in cfgs: raise HTTPException(404,"Config not found.")
    del cfgs[key]; write_configs(cfgs,cf)
    return{"message":"Deleted."}

@app.post("/humanize")
async def set_humanize(cfg:HumanizeConfig):
    app_state.set_jitter(cfg.jitter_strength); app_state.set_smooth(cfg.smooth_factor)
...
<div class="w">
  ...
  <div id="tab-recoil" class="tc active">
    ...
    <div class="card">
      ...
      <div class="hint" style="margin-bottom:9px">
        ...
        <span style="color:#223a28;">— — —</span> เส้นประเขียว = ค่าคงที่ที่ตั้งไว้ &nbsp; <span style="color:#3ab070;">——</span> เส้นฟ้าเขียว = curve ที่วาด
      </div>
      <div class="ceditor">
  ...
  <div id="tab-humanize" class="tc">
    <div class="card">
      ...
      <div class="hrow">
        ...
        <input type="range" id="ss" min="0" max="0.95" step="0.01" value="0.60" style="flex:1">
        <span class="hval" id="sv2">0.60</span>
      </div>
      <div class="hint">Exponential smoothing · 0 = ดิบ · 0.95 = นุ่มมาก</div>
    </div>
  </div>
  ...
  <div id="tab-settings" class="tc">
    ...
    <div class="card">
      <div class="clabel">Controller</div>
      <select id="ctrl">
    ...
    <div class="card" id="sw-card" style="display:none">
      ...
      <div class="hint" style="line-height:1.7">ctypes SendInput บน Windows — 1-PC setup<br>
      <span style="color:#252540;">⚠ ไม่ bypass kernel Anti-Cheat (BattleEye / EAC)</span></div>
    </div>
...
function connectWs(){
  ...
  ws.onopen=sendAll; ws.onclose=()=>setTimeout(connectWs,2000);
}
connectWs();

function sendAll(){
  if(ws&&ws.readyState===1) ws.send(JSON.stringify({
    pull_down:+sv.value||0, horizontal:+hv.value||0,
    horizontal_delay_ms:+dv.value||0, horizontal_duration_ms:+uv.value||0,
    vertical_delay_ms:+vdv.value||0, vertical_duration_ms:+vduv.value||0,
    jitter_strength:+jss.value||0, smooth_factor:+sss.value||0,
  }));
}

function sync(r,i,cb){
  r.oninput=()=>{i.value=r.value;if(cb)cb();sendAll();};
  i.oninput=()=>{r.value=i.value;if(cb)cb();sendAll();};
}
sync(sl,sv,drawCurve); sync(hs,hv); sync(ds,dv); sync(us,uv);
...
// ── Status ────────────────────────────────────────────────────────────────────
function setBtn(on){$('toggle-btn').textContent=on?'■ ON':'○ OFF';$('toggle-btn').className=on?'enabled':'disabled';}
$('toggle-btn').onclick=()=>fetch('/toggle',{method:'POST'}).then(r=>r.json()).then(d=>setBtn(d.is_enabled));
$('tbs').onchange=()=>fetch('/toggle-button',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({button:$('tbs').value})});
...
function getStatus(){
  fetch('/status').then(r=>r.json()).then(d=>{
    ...
    if(d.controller_type){$('ctrl').value=d.controller_type;ctrlUI(d.controller_type);}
...
$('ctrl').onchange=()=>{const ct=$('ctrl').value;fetch('/controller-type',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({controller:ct})}).then(()=>ctrlUI(ct));};
$('trig').onchange=()=>fetch('/trigger-mode',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({mode:$('trig').value})});
...
function resize(){
  ...
  cv.width=r.width||cv.offsetWidth||300; cv.height=120; drawCurve();
}
...
function drawCurve(){
  ...
  [1,2,3].forEach(i=>{ctx.beginPath();ctx.moveTo(0,H*i/4);ctx.lineTo(W,H*i/4);ctx.stroke();});
  [1,2,3].forEach(i=>{ctx.beginPath();ctx.moveTo(W*i/4,0);ctx.lineTo(W*i/4,H);ctx.stroke();});
  // Reference line (constant value)
  const refY=1-Math.min((+sv.value||0)/300,1);
...
$('curve-load-btn').onclick=()=>{
  const v=Math.max(0,Math.min(300,+sv.value||0));
  const N=40;
...
$('curve-clear-btn').onclick=()=>{
  ...
  if(ws&&ws.readyState===1) ws.send(JSON.stringify({pull_down_curve:null}));
};
...
function renderTagChips(){
  ...
  Object.entries(currentTags).forEach(([k,v])=>{
    ...
    chip.querySelector('.rm').onclick=e=>{delete currentTags[e.target.dataset.k];renderTagChips();updSavePreview();};
    c.appendChild(chip);
  });
}
...
function updSavePreview(){
  ...
  const vv=+sv.value||0, hh=+hv.value||0;
  txt+=`  ↓${vv}`;
  if(hh!==0) txt+=`  ←→${hh}`;
...
function fetchConfigs(){
  fetch('/configs').then(r=>r.json()).then(d=>{cache=d;allKeys=Object.keys(d);buildTagFilters();filterBrowse();}).catch(()=>{});
}

function buildTagFilters(){
  const tagSet=new Set();
  Object.values(cache).forEach(cfg=>{
    if(typeof cfg==='object'&&cfg.tags) Object.entries(cfg.tags).forEach(([k,v])=>tagSet.add(`${k}:${v}`));
  });
...
function filterBrowse(){
  ...
  for(const key of allKeys){
    ...
    o.value=key; o.textContent=name+(tagStr?' '+tagStr:'');
    $('cfgdd').appendChild(o);
  }
  $('cfgdd').value=prev; updBrowseLbl();
}
...
$('cfgdd').onchange=()=>{
  ...
  sendAll();
  if(ws&&ws.readyState===1) ws.send(JSON.stringify({pull_down_curve:hasCurve?cfg.pull_down_curve:null}));
  // Pre-fill save panel
  const name=typeof cfg==='object'?(cfg.name||key):key;
  $('cfg-name').value=name;
...
$('save-btn').onclick=()=>{
  ...
  fetch('/configs',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(buildPayload(name))})
    .then(r=>r.json()).then(d=>{if(d.detail)showWarn(d.detail);else fetchConfigs();})
...
$('delete-btn').onclick=()=>{
  ...
  fetch('/configs/'+encodeURIComponent(key),{method:'DELETE'}).then(r=>r.json()).then(()=>fetchConfigs()).catch(()=>{});
};

// ── Profile (config file) management ─────────────────────────────────────────
function fetchCfgFiles(){
  fetch('/config-files').then(r=>r.json()).then(d=>{
    ...
    dd.value=d.current; $('cfg-badge').textContent=d.current.replace('.json','');
  }).catch(()=>{});
}

$('cfgfd').onchange=()=>{
  fetch('/config-files/switch',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({filename:$('cfgfd').value})})
    .then(r=>r.json()).then(d=>{$('cfg-badge').textContent=d.current_config_file.replace('.json','');cache=d.guns;allKeys=Object.keys(d.guns);buildTagFilters();filterBrowse();})
    .catch(()=>{});
};
$('create-cfg').onclick=()=>{
  ...
  fetch('/config-files',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({filename:n})})
    .then(r=>r.json()).then(()=>{fetchCfgFiles();$('new-cfg-name').value='';}).catch(()=>{});
};
$('delete-cfg').onclick=()=>{
  const f=$('cfgfd').value;if(!f||f==='default.json')return;
  if(confirm('ลบ profile "'+f+'"?'))
    fetch('/config-files/'+encodeURIComponent(f),{method:'DELETE'}).then(r=>r.json()).then(()=>fetchCfgFiles()).catch(()=>{});
};
